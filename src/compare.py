def compare_torrents(session, host, user, password, cmp_host, cmp_user, cmp_pass):
    import requests, sys
    def get_hashes(host, user, password):
        s = requests.Session()
        try:
            login = s.post(f"{host}/api/v2/auth/login", data={"username": user, "password": password})
            ...
            return set()
    src_hashes = get_hashes(host, user, password)
    cmp_hashes = get_hashes(cmp_host, cmp_user, cmp_pass)
